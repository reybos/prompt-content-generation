...
/**
 * Run the complete Halloween Appear song generation pipeline (including titles)
    ...
    while (attempt < maxAttempts && !finished) {
      ...
      try {
        if (options.emitLog && options.requestId) {
          options.emitLog(`🎃 Generating Halloween Appear song with ${selectedStyle} style... (Attempt ${attempt})`, options.requestId);
        }

        // Step 1: Generate image prompts with Halloween Appear style
        if (options.emitLog && options.requestId) {
          options.emitLog(`🖼️ Generating image prompts for ${segments.length} segments using ${selectedStyle} style...`, options.requestId);
        }
        
        // Создаем промт с Halloween Appear стилем
        ...
        const imageJson: string | Record<string, any> | null = await executePipelineStep(
          'HALLOWEEN APPEAR IMAGE PROMPTS',
          imageChain,
          { songLyrics: lyrics }
        );
        let globalStyle = '';
        ...
        if (imageJson) {
          const parsed = typeof imageJson === 'string' ? safeJsonParse(imageJson, 'HALLOWEEN APPEAR IMAGE PROMPTS') : imageJson;
        ...
        try {
          ...
          if (videoJson) {
            const parsed = typeof videoJson === 'string' ? safeJsonParse(videoJson, 'HALLOWEEN APPEAR VIDEO PROMPTS') : videoJson;
            if (options.emitLog && options.requestId) {
        ...
        for (let segmentIndex = 0; segmentIndex < videoSegments.length; segmentIndex++) {
          ...
          try {
            const titleChain = createChain(halloweenAppearTitlePrompt, { model: titleModel, temperature: titleTemperature });
            
            // Логируем title промт в консоль
            halloweenAppearLogTitlePrompt(segmentLines, segmentVideoPrompts, globalStyle);
            
            titleJson = await executePipelineStep(
              'HALLOWEEN APPEAR TITLE',
              titleChain,
              { 
            ...
            if (titleJson) {
              const parsed = typeof titleJson === 'string' ? safeJsonParse(titleJson, 'HALLOWEEN APPEAR TITLE') : titleJson;
              if (parsed && typeof parsed === 'object') {
                title = parsed.title || '';
              }
            } else {
          ...
          try {
            ...
            for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
              ...
              while (imageAttempts < maxImageAttempts && !groupImagePrompt) {
                ...
                const groupImageChain = createChain(halloweenAppearGroupImagePrompt, { 
                  model: groupImageModel, 
                  temperature: groupImageTemperature 
                });
                
                logHalloweenAppearGroupImagePrompt(globalStyle, threePrompts);
                
                const groupImageJson: string | Record<string, any> | null = await executePipelineStep(
                  'HALLOWEEN APPEAR GROUP IMAGE',
                  groupImageChain,
                  { 
                    globalStyle: globalStyle,
                ...
                if (groupImageJson) {
                  const parsed = typeof groupImageJson === 'string' ? 
                    safeJsonParse(groupImageJson, 'HALLOWEEN APPEAR GROUP IMAGE') : groupImageJson;
              ...
              while (videoAttempts < maxVideoAttempts && !groupVideoPrompt) {
                ...
                const groupVideoChain = createChain(halloweenAppearGroupVideoPrompt, { 
                  model: groupVideoModel, 
                  temperature: groupVideoTemperature 
                });
                
                logHalloweenAppearGroupVideoPrompt(groupImagePrompt);
                
                const groupVideoJson: string | Record<string, any> | null = await executePipelineStep(
                  'HALLOWEEN APPEAR GROUP VIDEO',
                  groupVideoChain,
                  { 
                ...
                if (groupVideoJson) {
                  const parsed = typeof groupVideoJson === 'string' ? 
                    safeJsonParse(groupVideoJson, 'HALLOWEEN APPEAR GROUP VIDEO') : groupVideoJson;
