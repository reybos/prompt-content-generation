 ...
 */
export const songWithAnimalsGroupImagePrompt = PromptTemplate.fromTemplate(`
...
- prompts: three structured inputs, each containing a "line" and a detailed "prompt" describing one animal character  

Instructions:  
1. Carefully read the three input prompts and extract the key character details (appearance, animal features, colors, personality, and posture).  
2. Write a **single unified group prompt** in the same descriptive style as the examples.  
3. The output must:  
   - Describe all three animal characters together in one scene, each with their unique traits preserved.  
   - Explicitly state that all characters are **standing side by side on the ground** and **facing the camera**.  
   - Include environment details (playground, forest, farm, garden, etc.) consistent with the given global_style.  
   - End with a short "Environment" paragraph that sets the atmosphere, ground texture, lighting, and mood.  
   - Match the tone of the provided examples: cheerful and family-friendly, educational atmosphere.  
...
Rules:
	...
	•	Movements are minimal, calm, and natural (gentle breathing, slight body sway, slow head tilt, subtle ear or tail motions).
	•	Each character should have distinct but very simple movements that fit its animal nature.
	•	No blinking, no speaking, no large gestures, no rotation around their axis.
	•	Characters stay in place.
	•	Environment has soft background motion (leaves rustling, gentle breeze, soft lighting changes).
...
A group of three [style] animal characters stand together in [environment]:
